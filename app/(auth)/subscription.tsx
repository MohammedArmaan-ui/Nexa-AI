...
export default function SubscriptionScreen() {
  ...
  const renderedPlans = useMemo(() => plans.map(plan => (
    <View key={plan.id} style={styles.planCard}>
      ...
      {/*
          BOLT OPTIMIZATION: Consolidating multiple sibling Text components into a single
          multi-line Text component reduces the native Shadow Tree node count.
      */}
      <Text style={styles.feature}>{plan.features.map(feat => `• ${feat}`).join('\n')}</Text>
      <TouchableOpacity
        style={styles.button}
